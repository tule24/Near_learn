...
use near_sdk::{log, near_bindgen, env, AccountId, Gas, Promise, PromiseError, PanicOnDefault};
...
impl Contract {
    ...
    pub fn query_greeting_callback(&self, #[callback_result] call_result: Result<String, PromiseError>) -> String {
        // Check if the promise successed by calling the method outlined in external.rs
    ...
    pub fn change_greeting_callback(&mut self, #[callback_result] call_result: Result<(), PromiseError>) -> bool {
        // Check if the promise successed by calling the method outlined in external.rs
        if call_result.is_err() {
            env::log_str("set_greeting_false...");
            return false
        }

        env::log_str("set_greeting_success...");
        return true
    }
}
